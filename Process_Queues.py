...
def Proc_Queue(Reachability_Graph_Mat, Single_Reln_no_Conflict_Queue_process, Output_Text_File):
  ...
  while (0 < len(Inp_Queue)):
	...
	fp.write('\n ==>>>>>>>>> NEW CONN --- nodes to be connected: ' + str(src_taxa_label) + str(dest_taxa_label) + \
	      'nodes indices: ' + str(COMPLETE_INPUT_TAXA_LIST.index(src_taxa_label)) + str(COMPLETE_INPUT_TAXA_LIST.index(dest_taxa_label)) + \
