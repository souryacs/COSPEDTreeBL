...
def ComputeXLScore(clust_spec_list1, clust_spec_list2):
  ...
  for x in clust_spec_list1:
	...
	valid_taxa_pairs = valid_taxa_pairs + 1
  ...
  # comment - sourya
  # previously we returned the sum of average XL computed for all taxa pairs
  #return level_score
  
  # add - sourya
  # now we return normalized sum of average XL (by dividing with the number of valid taxa pairs)
  if (valid_taxa_pairs > 0):
    return ((level_score * 1.0) / valid_taxa_pairs)
  else:
    return 0
...
def ResolveMultifurcation(Curr_tree, clust_species_list, no_of_input_clusters, Output_Text_File):
  ...
  for i in range(no_of_clust - 1):
	...
	fp.close()
      ...
      Level_Score_Mat[j][i] = Level_Score_Mat[i][j] = ComputeXLScore(clust_species_list[i], clust_species_list[j])    
  ...
  while(no_of_clust > 2):      
    ...
    # with an exception handling case
    for i in range(no_of_clust - 1):
      for j in range(i+1, no_of_clust):
	if ((sum_list[i] + sum_list[j]) > 0):
	  Norm_Level_Score_Mat[j][i] = Norm_Level_Score_Mat[i][j] = (Level_Score_Mat[i][j] * 1.0) / (sum_list[i] + sum_list[j])
	else:
	  Norm_Level_Score_Mat[j][i] = Norm_Level_Score_Mat[i][j] = 0
